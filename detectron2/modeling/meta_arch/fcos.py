...
from detectron2.modeling.roi_heads.mask_head import build_mask_head, mask_rcnn_inference_amodal, mask_rcnn_loss
...
class FCOS(nn.Module):
    ...
    def forward(self, batched_inputs, c_iter=None):
        """
        Args:
        ...
        if self.training:
            return self._forward_train(
                ...
                locations, box_cls, 
                box_regression, 
                centerness, gt_instances, batched_inputs, images, c_iter
            )
        else:
            #assert False
            return self._forward_test(
                ...
                centerness, batched_inputs, images, c_iter
            )

    def _forward_train(self, features_list, locations, box_cls, box_regression, centerness, gt_instances, batched_inputs, images, c_iter):
        ...
        del gt_instances
        loss_mask, loss_mask_bo, loss_a_mask, loss_mask_1, loss_justify = self._forward_mask(features_list, proposals,  c_iter)
    ...
    def _forward_test(self, features, locations, box_cls, box_regression, centerness, batched_inputs, images, c_iter):
        ...
        instances = self._forward_mask(features, instances, c_iter)
        
        return self._postprocess(instances, batched_inputs, images.image_sizes)

    def _forward_mask(self, features, instances , c_iter):
        """
        ...
        if self.training:
            ...
            #    return 0.0, 0.0, 0.0
            #else:
            visible_mask, amodal_mask, inmodal_mask, occluder_mask, invisible_mask = self.mask_head(mask_features, c_iter)
            ...
            loss_mask, loss_mask_bo, loss_boundary, loss_boundary_bo, loss_a_mask, loss_a_boundary, loss_justify =\
                mask_rcnn_loss(inmodal_mask, boundary, proposals, occluder_mask, bo_bound, 
                                ...
                                c_iter=c_iter, dice_loss=False, pred_invisible_mask_logits=invisible_mask)
            ...
            loss_mask_1, loss_mask_bo_1, loss_boundary_1, loss_boundary_bo_1, loss_a_mask_1, loss_a_boundary_1, loss_justify_1 =\
                mask_rcnn_loss(visible_mask, boundary, proposals, occluder_mask_1, bo_bound, 
                                use_i_mask=self.aisformer.USE, pred_a_mask_logits=amodal_mask_1, 
                                pred_a_boundary_logits=a_mask_bound,
                                use_justify_loss=self.aisformer.JUSTIFY_LOSS,
                                c_iter=c_iter, dice_loss=False, pred_invisible_mask_logits=invisible_mask_1)

            return loss_mask, loss_mask_bo, loss_a_mask, loss_mask_1, loss_justify
        else:
